...
async function ipoNewsModule({ market = 'all', limit = 30, detailTop = 14 } = {}) {
  ...
  Object.values(ipo).forEach((mod) => {
    (mod.list || []).forEach((x) => {
      ...
      targets.push({
        ...
        subscribeStart: x.subscribeStart, listingDate: x.listingDate, priceText: x.priceText,
      });
    });
  });
  ...
  const details = await Promise.allSettled(detailTargets.map((x) => W.newsDetail(`nes${x.id}`)));
  details.forEach((r, i) => {
